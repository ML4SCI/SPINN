...
mesh = pv.get_reader("paul_trap_basis_DC_world_boundary_contact.dat").read()[0]
mesh = mesh.cell_data_to_point_data()

mesh["phi_first_contact"] = data_DC["phi_first_contact"]
mesh["phi_third_contact"] = data_DC["phi_third_contact"]
mesh["phi_fifth_contact"] = data_DC["phi_fifth_contact"]
mesh["phi_world_boundary_contact"] = data_DC["phi_world_boundary_contact"]
mesh["phi_dc_total"] = North_contact_voltage*data_DC["phi_first_contact"] + North_contact_voltage*data_DC["phi_third_contact"] + North_contact_voltage*data_DC["phi_fifth_contact"] + data_DC["phi_world_boundary_contact"]

# ----------------------------
# separately load RF solve to get field data
# ----------------------------

rf_mesh = pv.get_reader("paul_trap_basis_RF.dat").read()[0]
...
# mesh["psi_total"] = mesh["phi_dc_total"]
...
contours = mesh.contour(isosurfaces=1000, scalars="psi_total_eV")

p = pv.Plotter()
p.add_mesh(mesh, scalars="psi_total_eV", cmap="jet", opacity=0.6)
p.add_mesh(contours, color="black", line_width=0.5)
p.view_xy()
p.show()
